...
@Component({
    selector: 'jhi-house-hold-wl',
    templateUrl: './house-hold-wl.component.html'
})
export class HouseHoldWlComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/house-hold-wl'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/house-hold-wl', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: HouseHoldWl) {
        return item.id;
    }
